...
import static com.jobboard.utils.MasterData.getJobDTOPage;
...
public class JobControllerTest {
	...
	public void testGetAllJobs() throws Exception {
		Page<JobDTO> jobDTOS = getJobDTOPage(0, 10);

		when(this.jobService.getAllJobs(any(Pageable.class))).thenReturn(jobDTOS);
	...
	@Test
	public void testGetJobsByLocation() throws Exception {
		String location = "TestLocation";
		List<JobDTO> jobs = getJobDTOList(); // You need to define getJobDTOList() method

		when(this.jobService.getJobsByLocation(eq(location))).thenReturn(jobs);
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/api/jobs/byLocation").param("location", location)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON);
